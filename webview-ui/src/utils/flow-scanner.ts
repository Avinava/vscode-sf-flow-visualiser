        ...
        return new Promise((resolve) => {
            ...
            const handleMessage = (event: MessageEvent) => {
                ...
                if (message.command === "flowAnalysisResult") {
                    console.log("[FlowScanner] Received result from extension:", message.payload);
                    ...
                    if (result.success) {
                        ...
                        console.error("[FlowScanner] Analysis failed:", result.errorMessage);
                        resolve({
                            cyclomaticComplexity: 0,
                            violations: [],
                            violationsByRule: {},
            ...
            timeoutId = window.setTimeout(() => {
                ...
                window.removeEventListener("message", handleMessage);
                console.warn("[FlowScanner] Analysis timeout - returning empty results");
                resolve({
                    cyclomaticComplexity: 0,
                    violations: [],
                    violationsByRule: {},
 ...
 */
function groupViolationsByRule(violations: FlowViolation[]): Record<string, FlowViolation[]> {
...
export function getQualitySummary(metrics: FlowQualityMetrics): string {
    ...
    if (violationsBySeverity.error > 0) {
        parts.push(`${violationsBySeverity.error} error${violationsBySeverity.error > 1 ? "s" : ""}`);
    }
    if (violationsBySeverity.warning > 0) {
        parts.push(`${violationsBySeverity.warning} warning${violationsBySeverity.warning > 1 ? "s" : ""}`);
    }
    if (violationsBySeverity.note > 0) {
        parts.push(`${violationsBySeverity.note} note${violationsBySeverity.note > 1 ? "s" : ""}`);
    }
