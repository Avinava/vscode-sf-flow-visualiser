...
const RULE_METADATA: Record<string, { label: string; description: string; docAnchor: string }> = {
    CyclomaticComplexity: {
        label: "Cyclomatic Complexity",
        description: "Flow complexity is too high. Consider breaking into smaller subflows.",
        docAnchor: "cyclomatic-complexity",
    },
    DMLStatementInLoop: {
        label: "DML Statement In Loop",
        description: "Database operations inside loops can exceed governor limits. Move DML outside loops.",
        docAnchor: "dml-statement-in-a-loop",
    },
    SOQLQueryInLoop: {
        label: "SOQL Query In Loop",
        description: "SOQL queries inside loops can exceed governor limits. Move queries outside loops.",
        docAnchor: "soql-query-in-a-loop",
    },
    HardcodedId: {
        label: "Hardcoded ID",
        description: "Hardcoded IDs are org-specific and will break in other environments.",
        docAnchor: "hardcoded-id",
    },
    HardcodedUrl: {
        label: "Hardcoded URL",
        description: "Hardcoded URLs are environment-specific. Use $API formula or custom labels.",
        docAnchor: "hardcoded-url",
    },
    MissingFaultPath: {
        label: "Missing Fault Path",
        description: "Element lacks error handling. Add fault connectors for graceful error handling.",
        docAnchor: "missing-fault-path",
    },
    MissingNullHandler: {
        label: "Missing Null Handler",
        description: "Decision doesn't handle null/empty values. Add a null check to prevent runtime errors.",
        docAnchor: "missing-null-handler",
    },
    DuplicateDMLOperation: {
        label: "Duplicate DML Operation",
        description: "Duplicate database operations between screens. Prevent users from navigating backward.",
        docAnchor: "duplicate-dml-operation",
    },
    APIVersion: {
        label: "Outdated API Version",
        description: "Flow uses an outdated API version. Update to the latest version for best compatibility.",
        docAnchor: "outdated-api-version",
    },
    FlowName: {
        label: "Flow Naming Convention",
        description: "Flow name doesn't follow naming conventions. Use a clear, descriptive name with domain prefix.",
        docAnchor: "flow-naming-convention",
    },
    UnconnectedElement: {
        label: "Unconnected Element",
        description: "Element is not connected to the flow. Remove unused elements or connect them properly.",
        docAnchor: "unconnected-element",
    },
    MissingFlowDescription: {
        label: "Missing Flow Description",
        description: "Flow lacks a description. Add documentation to help others understand its purpose.",
        docAnchor: "missing-flow-description",
    },
    CopyAPIName: {
        label: "Copy API Name",
        description: "Element has a 'Copy_X_Of_' name pattern. Rename copied elements for clarity.",
        docAnchor: "copy-api-name",
    },
    GetRecordAllFields: {
        label: "Get Record All Fields",
        description: "Using 'Get all fields' violates principle of least privilege. Specify only needed fields.",
 ...
 */
export async function analyzeFlowXML(flowXml: string): Promise<FlowScannerResult> {
    ...
    try {
        ...
        const config: any = {
            ...
            MissingFaultPath: { severity: "warning" },
        ...
        console.log("[Extension FlowScanner] Config:", JSON.stringify(config, null, 2));
        console.log("[Extension FlowScanner] Flows array:", flows);

        // Run the scanner
        const scanResults = await scan(flows, config);
        console.log(`[Extension FlowScanner] Scan found ${scanResults.length} result(s)`);
        console.log("[Extension FlowScanner] Scan results detail:", JSON.stringify(scanResults, null, 2));
        ...
        for (const result of scanResults) {
            ...
            console.log("[Extension FlowScanner] Rule results count:", result.ruleResults?.length);
            ...
            for (const ruleResult of result.ruleResults) {
                console.log("[Extension FlowScanner] Rule result:", ruleResult.ruleName, "occurs:", ruleResult.occurs, "details count:", ruleResult.details?.length);
        ...
        console.log(`[Extension FlowScanner] Analysis complete: ${violations.length} violations found`);
        ...
        if (tmpFilePath) {
            try {
                ...
                console.warn("[Extension FlowScanner] Failed to cleanup temp file:", cleanupError);
            }
        }
    }
}
